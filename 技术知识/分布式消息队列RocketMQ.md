![](media/9dae6175e622adceffa5b973c3d62631.png)
...
![](media/ccee6e0adda6962118902a7067bd18f8.png)
...
![](media/c7eeff7352cd72db9c8f368d77c4e5d7.png)2012年，MetaQ发展到了v3.0版本，在它基础上进行了进一步的抽象，形成了RocketMQ，然后就将其 进行了开源。
...
![](media/831ec3acf25fb219cbebc0b4fe309c3d.png)
...
### ![](media/1381034f33d85efdae4cd343bbb71b3f.png)路由发现
...
![](media/fae49cbedee17dbd8eeb86dc49b3b065.png)
...
## ![](media/574208f5351376cd31758ebfa0e126d7.png)Broker集群模式

根据Broker集群中各个节点间关系的不同，Broker集群可以分为以下几类： **单Master**
...
![](media/966fe2b039671b7e893043cdab5adc6e.png)该模式与异步复制模式相比，优点是消息的安全性更高，不存在消息丢失的情况。但单个消息的RT略 高，从而导致性能要略低（大约低10%）。

该模式存在一个大的问题：对于目前的版本，Master宕机后，Slave 不会自动切换到Master。**最佳实践**
...
![](media/a4dda5e1156d188cc3b4cc8697c62971.png)RAID 每一个等级代表一种实现方法和技术，等级之间并无高低之分。在实际应用中，应当根据用户的数据应用特点，综合考虑可用性、性能和成本来选择合适的 RAID 等级，以及具体的实现方式。
...
### ![](media/ebd1c6f8cc59e0041bf86621a1e976c8.jpeg)RAID10
...
11. \# 指定Broker对外提供服务的端口，即Broker与producer与consumer通信的端口。默认10911。由于当前主机同时充当着master1与slave2，而前面的master1使用的是默认端口。这 里需要将这两个端口加以区分，以区分出master1与slave2
12. listenPort=11911
13. \# 指定消息存储相关的路径。默认路径为\~/store目录。由于当前主机同时充当着master1与

    slave2，master1使用的是默认路径，这里就需要再指定一个不同路径

14. storePathRootDir=\~/store-s
15. storePathCommitLog=\~/store-s/commitlog
16. storePathConsumeQueue=\~/store-s/consumequeue
17. storePathIndex=\~/store-s/index
18. storeCheckpoint=\~/store-s/checkpoint
19. abortFile=\~/store-s/abort
...
2.  brokerId=0
3.  \#nameServer地址，分号分割
4.  namesrvAddr=nameserver1:9876;nameserver2:9876
5.  \#默认为新建Topic所创建的队列数
6.  defaultTopicQueueNums=4
7.  \#是否允许 Broker 自动创建Topic，建议生产环境中关闭
8.  autoCreateTopicEnable=true
9.  \#是否允许 Broker 自动创建订阅组，建议生产环境中关闭
10. autoCreateSubscriptionGroup=true
11. \#Broker对外提供服务的端口，即Broker与producer与consumer通信的端口
12. listenPort=10911
13. \#HA高可用监听端口，即Master与Slave间通信的端口，默认值为listenPort+1
14. haListenPort=10912
15. \#指定删除消息存储过期文件的时间为凌晨4点
16. deleteWhen=04
17. \#指定未发生更新的消息存储文件的保留时长为48小时，48小时后过期，将会被删除
18. fileReservedTime=48
19. \#指定commitLog目录中每个文件的大小，默认1G
20. mapedFileSizeCommitLog=1073741824
21. \#指定ConsumeQueue的每个Topic的每个Queue文件中可以存放的消息数量，默认30w条
22. mapedFileSizeConsumeQueue=300000
23. \#在清除过期文件时，如果该文件被其他线程所占用（引用数大于0，比如读取消息），此时会阻止 此次删除任务，同时在第一次试图删除该文件时记录当前时间戳。该属性则表示从第一次拒绝删除 后开始计时，该文件最多可以保留的时长。在此时间内若引用数仍不为0，则删除仍会被拒绝。不过 时间到后，文件将被强制删除
24. destroyMapedFileIntervalForcibly=120000
25. \#指定commitlog、consumequeue所在磁盘分区的最大使用率，超过该值，则需立即清除过期文 件
26. diskMaxUsedSpaceRatio=88
27. \#指定store目录的路径，默认在当前用户主目录中
28. storePathRootDir=/usr/local/rocketmq-all-4.5.0/store
29. \#commitLog目录路径
...
![](media/35dce2db90c3b3a6f2fd5e966da20e03.png)46 \# -
...
![](media/681a7c91d932112fca0352a881a0f355.png)8
...
1.  ![](media/794d116277db85eab2755ba0df7027ac.png) consumerConnection Query consumer's socket connection, client version and subscription
2.  consumerProgress Query consumers's progress, speed
1.  ![](media/fada4d1082d1a754b5bffd53ff9e0571.png)consumerStatus Query consumer's internal data structure
...
13. statsAll Topic and Consumer tps stats
...
![](media/058f13dc2cc8c055900fa89dff550f62.png)Consumer向Broker发送拉取请求，其中会包含其要拉取消息的Queue、消息offset及消息Tag。
...
### ![](media/ae0d06163b8f048eae84378a09c442f0.jpeg)消息进度保存
...
Rebalance机制的本意是为了提升消息的并行消费能力。例如，⼀个Topic下5个队列，在只有1个消费 者的情况下，这个消费者将负责消费这5个队列的消息。如果此时我们增加⼀个消费者，那么就可以给 其中⼀个消费者分配2个队列，给另⼀个分配3个队列，从而提升消息的并行消费能力。
...
![](media/acf0edfd93328e6b89d2b3889018136f.png)消费突刺：由于Rebalance可能导致重复消费，如果需要重复消费的消息过多，或者因为Rebalance暂停 时间过长从而导致积压了部分消息。那么有可能会导致在Rebalance结束之后瞬间需要消费很多消息。
...
## ![](media/ff26d58ce79343914790f38a5653518c.jpeg)至少一次原则
...
![](media/c19d0b4a2e20d5422a8b445042accf7d.png)
...
![](media/d88cc7326a626b9ebf97e2c535b9660c.png)当出现消费者对某条消息重复消费的情况时，重复消费的结果与消费一次的结果是相同的，并且多次消 费并未对业务系统产生任何负面影响，那么这个消费过程就是消费幂等的。
...
### ![](media/db9df86d5d07448e8d2047a1b658e8fd.png)消息拉取

Consumer通过长轮询Pull模式批量拉取的方式从服务端获取消息，将拉取到的消息缓存到本地缓冲队 列中。对于拉取式消费，在内网环境下会有很高的吞吐量，所以这一阶段一般不会成为消息堆积的瓶 颈。

一个单线程单分区的低规格主机*(Consumer*，*4C8G)*，其可达到几万的*TPS*。如果是多个分区多 个线程，则可以轻松达到几十万的*TPS*。

### 消息消费

Consumer将本地缓存的消息提交到消费线程中，使用业务消费逻辑对消息进行处理，处理完毕后获取 到一个结果。这是真正的消息消费过程。此时Consumer的消费能力就完全依赖于消息的消费耗时和消 费并发度了。如果由于业务处理逻辑复杂等原因，导致处理单条消息的耗时较长，则整体的消息吞吐 量肯定不会高，此时就会导致Consumer本地缓冲队列达到上限，停止从服务端拉取消息。
...
读写外部数据库，例如对远程*MySQL*的访问读写外部缓存系统，例如对远程*Redis*的访问

下游系统调用，例如*Dubbo*的*RPC*远程调用，*Spring Cloud*的对下游系统的*Http*接口调用

关于下游系统调用逻辑需要进行提前梳理，掌握每个调用操作预期的耗时，这样做是为了能够 判断消费逻辑中*IO*操作的耗时是否合理。通常消息堆积是由于下游系统出现了服务异常或达到 了DBMS容量限制，导致消费耗时增加。
...
![](media/deb5c81f9f8db3da6df3b9d8cad70cb0.png)一般情况下，消费者端的消费并发度由单节点线程数和节点数量共同决定，其值为单节点线程数\* 节点数量。不过，通常需要优先调整单节点的线程数，若单机硬件资源达到了上限，则需要通过横向扩展 来提高消费并发度。

单节点线程数，即单个*Consumer*所包含的线程数量节点数量，即*Consumer Group*所包含的*Consumer*数量

对于普通消息、延时消息及事务消息，并发度计算都是单节点线程数*\** 节点数量。但对于顺序消息则是不同的。顺序消息的消费并发度等于Topic的Queue分区数量。

1.  全局顺序消息：该类型消息的*Topic*只有一个*Queue*分区。其可以保证该*Topic*的所有消息被顺序消费。为了保证这个全局顺序性，*Consumer Group*中在同一时刻只能有一个*Consumer*的一个线程进行消费。所以其并发度为*1*。
1.  分区顺序消息：该类型消息的*Topic*有多个*Queue*分区。其仅可以保证该*Topic*的每个*Queue* 分区中的消息被顺序消费，不能保证整个*Topic*中消息的顺序消费。为了保证这个分区顺序性， 每个*Queue*分区中的消息在*Consumer Group*中的同一时刻只能有一个*Consumer*的一个线程进行消费。即，在同一时刻最多会出现多个*Queue*分蘖有多个*Consumer*的多个线程并行消费。所以其并发度为*Topic*的分区数量。
...
通过压测获取消息的消费耗时，并对耗时较高的操作的代码逻辑进行分析。梳理消息的消费耗时需要关 注以下信息：

消息消费逻辑的计算复杂度是否过高，代码是否存在无限循环和递归等缺陷。

![](media/de0dc4c0cfec10bac10f2c637db5a2eb.png)消息消费逻辑中的I/O操作是否是必须的，能否用本地缓存等方案规避。
...
1.  对于*RocketMQ*系统来说，删除一个*1G*大小的文件，是一个压力巨大的*IO*操作。在删除过程中，系统性能会骤然下降。所以，其默认清理时间点为凌晨*4*点，访问量最小的时间。也正因如 果，我们要保障磁盘空间的空闲率，不要使系统出现在其它时间点删除*commitlog*文件的情况。
1.  官方建议*RocketMQ*服务的*Linux*文件系统采用*ext4*。因为对于文件删除操作，*ext4*要比*ext3*性 能更好

**第4章 RocketMQ应用**一、普通消息

## 消息发送分类

![](media/a58f76367c86713456760d7332816ce6.png)Producer对于消息的发送方式也有多种选择，不同的方式会产生不同的系统效果。**同步发送消息**
...
![](media/7e8217b787cd76259f868885dfe834aa.png)
...
### ![](media/f9c79833ddbe39db984e22f79a334857.png)创建工程

创建一个Maven的Java工程rocketmq-test。**导入依赖**
...
1.  public class SyncProducer {
    ...
    2.  // 创建一个producer，参数为Producer Group名称
    3.  DefaultMQProducer producer = new DefaultMQProducer("pg");
    4.  // 指定nameServer地址
    5.  producer.setNamesrvAddr("rocketmqOS:9876");
    6.  // 设置当发送失败时重试发送的次数，默认为2次
    7.  producer.setRetryTimesWhenSendFailed(3);
    8.  // 设置发送超时时限为5s，默认3s
    9.  producer.setSendMsgTimeout(5000);
...
4.  byte[] body = ("Hi," + i).getBytes();
5.  Message msg = new Message("someTopic", "someTag", body);
6.  // 为消息指定key
7.  msg.setKeys("key-" + i);
8.  // 发送消息
...
// 关 闭 producer producer.shutdown();
...
3.  SEND_OK, // 发送成功
4.  FLUSH_DISK_TIMEOUT, // 刷盘超时。当Broker设置的刷盘策略为同步刷盘时才可能出现这种异常状态。异步刷盘不会出现
5.  FLUSH_SLAVE_TIMEOUT, // Slave同步超时。当Broker集群设置的Master-Slave的复制方式为同步复制时才可能出现这种异常状态。异步复制不会出现
6.  SLAVE_NOT_AVAILABLE, // 没有可用的Slave。当Broker集群设置为Master-Slave的复制方式为同步复制时才可能出现这种异常状态。异步复制不会出现
...
1.  public class AsyncProducer {
    ...
    3.  producer.setNamesrvAddr("rocketmqOS:9876");
    4.  // 指定异步发送失败后不进行重试发送
    5.  producer.setRetryTimesWhenSendAsyncFailed(0);
    6.  // 指定新创建的Topic的Queue数量为2，默认为4
    7.  producer.setDefaultTopicQueueNums(2); 9

        10 producer.start(); 11

        12 for (int i = 0; i \< 100; i++) {

2.  byte[] body = ("Hi," + i).getBytes();
3.  try {
1.  Message msg = new Message("myTopicA", "myTag",
2.  // 异步发送。指定回调
3.  producer.send(msg, new SendCallback() {

    body);

![](media/46da3e3c99345831364c8dfffa7caeb1.png)18
...
![](media/0a6a7015afd9804bf79ba90c91e4a70b.png)
...
2.  byte[] body = ("Hi," + i).getBytes();
3.  Message msg = new Message("single", "someTag", body);
4.  // 单向发送
5.  producer.sendOneway(msg); 12 }
6.  producer.shutdown();
7.  System.out.println("producer shutdown"); 15 }
...
1 public class SomeConsumer { 2

1.  public static void main(String[] args) throws MQClientException {
2.  // 定义一个pull消费者
3.  // DefaultLitePullConsumer consumer = new

    DefaultLitePullConsumer("cg");

4.  // 定义一个push消费者
5.  DefaultMQPushConsumer consumer = new

    DefaultMQPushConsumer("cg");

6.  // 指定nameServer
7.  consumer.setNamesrvAddr("rocketmqOS:9876");
8.  // 指定从第一条消息开始消费

![](media/ef39b5d3434f66af0355f3a1a66d649b.png)11
...
1.  // 指定消费topic与tag
2.  consumer.subscribe("someTopic", "\*");
3.  // 指定采用“广播模式”进行消费，默认为“集群模式”
...
7.  // 一旦broker中有了其订阅的消息就会触发该方法的执行，
8.  // 其返回值为当前consumer消费的状态
9.  @Override
...
13. System.out.println(msg); 28 }
14. // 返回消费状态：消费成功
15. return ConsumeConcurrentlyStatus.CONSUME_SUCCESS; 31 }
...
16. // 开启消费者消费
17. consumer.start();
18. System.out.println("Consumer Started"); 37 }

    38 }

![](media/27be66a843fb8143d169876b616a2985.png)
...
![](media/8c5ebd29996c4996a9e1f716f1fc2230.png)

这种情况下，我们希望Consumer消费消息的顺序和我们发送是一致的，然而上述MQ的投递和消费方 式，我们无法保证顺序是正确的。对于顺序异常的消息，Consumer即使设置有一定的状态容错，也不 能完全处理好这么多种随机出现组合情况。

基于上述的情况，可以设计如下方案：对于相同订单号的消息，通过一定的策略，将其放置在一个Queue中，然后消费者再采用一定的策略（例如，一个线程独立处理一个queue，保证处理消息的顺序 性），能够保证消费的顺序性。

## 有序性分类

根据有序范围的不同，RocketMQ可以严格地保证两种消息的有序性：分区有序与全局有序。

### ![](media/92cd0feebdd2576f482c13b067c8dd50.png)全局有序

当发送和消费参与的Queue只有一个时所保证的有序是整个Topic中消息的顺序， 称为全局有序。

在创建*Topic*时指定*Queue*的数量。有三种指定方式：

*1*）在代码中创建*Producer*时，可以指定其自动创建的*Topic*的*Queue*数量*2*）在*RocketMQ*可视化控制台中手动创建*Topic*时指定*Queue*数量
...
![](media/e2d96d9351c4388bc3207be2ca2cd239.png)一般性的选择算法是，让选择*key*（或其*hash*值）与该*Topic*所包含的*Queue*的数量取模，其结果即为选择出的*Queue*的*QueueId*。
...
以上做法会不会出现如下新的问题呢？不属于那个*Consumer*的消息被拉取走了，那么应该消费 该消息的*Consumer*是否还能再消费该消息呢？同一个*Queue*中的消息不可能被同一个*Group*中的不同*Consumer*同时消费。所以，消费现一个*Queue*的不同选择*key*的消息的*Consumer*一定属于不同的*Group*。而不同的*Group*中的*Consumer*间的消费是相互隔离的，互不影响的。
...
2.  Integer orderId = i;
3.  byte[] body = ("Hi," + i).getBytes();
4.  Message msg = new Message("TopicA", "TagA", body);
5.  SendResult sendResult = producer.send(msg, new MessageQueueSelector() {
6.  @Override
7.  public MessageQueue select(List\<MessageQueue\> mqs, Message msg, Object arg) {
8.  Integer id = (Integer) arg;
9.  int index = id % mqs.size();
10. return mqs.get(index); 17 }

    18 }, orderId);

    19

    20 System.out.println(sendResult); 21 }

    22 producer.shutdown(); 23 }
...
![](media/681a7c91d932112fca0352a881a0f355.png)当消息写入到Broker后，在指定的时长后才可被消费处理的消息，称为延时消息。

采用RocketMQ的延时消息可以实现定时任务的功能，而无需使用定时器。典型的应用场景是，电商交 易中超时未支付关闭订单的场景，12306平台订票超时未支付取消订票的场景。
...
RocketMQ服务端的MessageStoreConfig 类中的如下变量中：
...
![](media/88506fff026dbd057ddf48125f0f6b7b.png)
...
![](media/a7ffc6c3089251e1a111c3ae9dab2577.png)根据延时等级，在consumequeue目录中SCHEDULE_TOPIC_XXXX主题下创建出相应的queueId 目录与consumequeue文件（如果没有这些目录与文件的话）。
...
是按照消息投递时间排序的。一个*Broker*中同一等级的所有延时消息会被写入到*consumequeue* 目录中*SCHEDULE_TOPIC_XXXX*目录下相同*Queue*中。即一个*Queue*中消息投递时间的延迟等 级时间是相同的。那么投递时间就取决于于消息存储时间了。即按照消息被发送到*Broker*的时

间进行排序的。

### 投递延时消息

Broker内部有⼀个延迟消息服务类ScheuleMessageService，其会消费SCHEDULE_TOPIC_XXXX中的消息，即按照每条消息的投递时间，将延时消息投递到⽬标Topic中。不过，在投递之前会从commitlog 中将原来写入的消息再次读出，并将其原来的延时等级设置为0，即原消息变为了一条不延迟的普通消 息。然后再次将消息投递到目标Topic中。
...
这其实就是一次普通消息发送。只不过这次的消息*Producer*是延迟消息服务类

*ScheuleMessageService*。

## ![](media/7df46d79d3bdae77659b333aa48b672e.png)代码举例
...
2.  byte[] body = ("Hi," + i).getBytes();
3.  Message msg = new Message("TopicB", "someTag", body);
4.  // 指定消息延迟等级为3级，即延迟10s
5.  // msg.setDelayTimeLevel(3);
6.  SendResult sendResult = producer.send(msg);
7.  // 输出消息被发送的时间
8.  System.out.print(new SimpleDateFormat("mm:ss").format(new Date()));
9.  System.out.println(" ," + sendResult); 16 }

    17 producer.shutdown(); 18 }
...
3.  DefaultMQPushConsumer consumer = new DefaultMQPushConsumer("cg");
4.  consumer.setNamesrvAddr("rocketmqOS:9876"); 5
...
8.  // 输出消息被消费的时间
1.  ![](media/39d63e2a33418ab3ebda07f3eecacc15.png)System.out.print(new

    SimpleDateFormat("mm:ss").format(new Date()));

2.  System.out.println(" ," + msg); 17 }

    18 return ConsumeConcurrentlyStatus.CONSUME_SUCCESS; 19 }

    20 });

3.  consumer.start();
4.  System.out.println("Consumer Started"); 23 }
...
![](media/3a1c3056af309cc7f337169219aff31f.png)

1.  工行系统发送一个给B增款1万元的同步消息M给Broker
    1.  消息被Broker成功接收后，向工行系统发送成功ACK
    2.  工行系统收到成功ACK后从用户A中扣款1万元
    3.  建行系统从Broker中获取到消息M
    4.  建行系统消费消息M，即向用户B中增加1万元

这其中是有问题的：若第*3*步中的扣款操作失败，但消息已经成功发送到了*Broker*。对于*MQ*来 说，只要消息写入成功，那么这个消息就可以被消费。此时建行系统中用户*B*增加了*1*万元。出 现了数据不一致问题。

## 解决思路

解决思路是，让第1、2、3步具有原子性，要么全部成功，要么全部失败。即消息发送成功后，必须要 保证扣款成功。如果扣款失败，则回滚发送成功的消息。而该思路即使用事务消息。这里要使用分布 式事务解决方案。

![](media/7d689e57f3c326ad4f05b0dfbae7ae71.png)

使用事务消息来处理该需求场景：

1.  事务管理器TM向事务协调器TC发起指令，开启全局事务
    1.  工行系统发一个给B增款1万元的事务消息M给TC
    2.  TC会向Broker发送半事务消息prepareHalf ，将消息M 预提交到Broker。此时的建行系统是看

        不到Broker中的消息M的

    3.  Broker会将预提交执行结果Report给TC。
    4.  如果预提交失败，则TC会向TM上报预提交失败的响应，全局事务结束；如果预提交成功，TC会 调用工行系统的回调操作，去完成工行用户A的预扣款1万元的操作
    5.  工行系统会向TC发送预扣款执行结果，即本地事务的执行状态
    6.  TC收到预扣款执行结果后，会将结果上报给TM。

预扣款执行结果存在三种可能性：

1.  // 描述本地事务执行状态
    1.  public enum LocalTransactionState {
        1.  COMMIT_MESSAGE, // 本地事务执行成功
        2.  ROLLBACK_MESSAGE, // 本地事务执行失败
        3.  UNKNOW, // 不确定，表示需要进行回查以确定本地事务的执行结果6 }
1.  ![](media/9522f031a9ee02a04b5e3711836ba729.png)TM会根据上报结果向TC发出不同的确认指令

若预扣款成功（本地事务状态为COMMIT_MESSAGE），则TM向TC发送Global Commit指令若预扣款失败（本地事务状态为ROLLBACK_MESSAGE），则TM向TC发送Global Rollback指令
...
以上方案并不是一个典型的*XA*模式。因为*XA*模式中的分支事务是异步的，而事务消息方案中的 消息预提交与预扣款操作间是同步的。
...
![](media/714bf80cf1c9008f531ce1b3c034e46f.png)1
...
1.  回调操作返回*UNKNWON*
2.  *TC*没有接收到*TM*的最终全局事务确认指令**RocketMQ中的消息回查设置**

    关于消息回查，有三个常见的属性设置。它们都在broker加载的配置文件中设置，例如：

![](media/acc8b092fbb05166f0ce81ee80dfd1ad.png)transactionTimeout=20，指定TM在20秒内应将最终确认状态发送给TC，否则引发消息回查。默 认为60秒

transactionCheckMax=5，指定最多回查5次，超过后将丢弃消息并记录错误日志。默认15次。transactionCheckInterval=10，指定设置的多次消息回查的时间间隔为10秒。默认为60秒。

## XA模式三剑客

### XA协议

XA（Unix Transaction）是一种分布式事务解决方案，一种分布式事务处理模式，是基于XA协议的。XA协议由Tuxedo（Transaction for Unix has been Extended for Distributed Operation，分布式操作扩展之后的Unix事务系统）首先提出的，并交给X/Open组织，作为资源管理器与事务管理器的接口标
...
![](media/9a0ea5dcf81cd2b3705a878ae8e54ecd.jpeg)

XA模式是一个典型的2PC，其执行原理如下：

1.  TM向TC发起指令，开启一个全局事务。
    1.  根据业务要求，各个RM会逐个向TC注册分支事务，然后TC会逐个向RM发出预执行指令。
    2.  各个RM在接收到指令后会在进行本地事务预执行。
    3.  RM将预执行结果Report给TC。当然，这个结果可能是成功，也可能是失败。
    4.  TC在接收到各个RM的Report后会将汇总结果上报给TM，根据汇总结果TM会向TC发出确认指令。

        若所有结果都是成功响应，则向TC发送Global Commit指令。只要有结果是失败响应，则向TC发送Global Rollback指令。
...
### ![](media/f6bb1f8231086044bbc479621e0b61d9.png)定义工行事务监听器
...
1.  // 回调操作方法
2.  // 消息预提交成功就会触发该方法的执行，用于完成本地事务
3.  @Override
4.  public LocalTransactionState executeLocalTransaction(Message msg, Object arg) {
5.  System.out.println("预提交消息成功：" + msg);
6.  // 假设接收到TAGA的消息就表示扣款操作成功，TAGB的消息表示扣款失败，
7.  // TAGC表示扣款结果不清楚，需要执行消息回查
8.  if (StringUtils.equals("TAGA", msg.getTags())) {
9.  return LocalTransactionState.COMMIT_MESSAGE;
10. } else if (StringUtils.equals("TAGB", msg.getTags())) {
11. return LocalTransactionState.ROLLBACK_MESSAGE;
12. } else if (StringUtils.equals("TAGC", msg.getTags())) {
13. return LocalTransactionState.UNKNOW; 16 }

    17 return LocalTransactionState.UNKNOW; 18 }
...
15. // 引发消息回查的原因最常见的有两个：
16. // 1)回调操作返回UNKNWON
17. // 2)TC没有接收到TM的最终全局事务确认指令
18. @Override
19. public LocalTransactionState checkLocalTransaction(MessageExt msg) {
20. System.out.println("执行消息回查" + msg.getTags());
21. return LocalTransactionState.COMMIT_MESSAGE; 28 }
...
1.  public class TransactionProducer {
    1.  public static void main(String[] args) throws Exception {
    2.  TransactionMQProducer producer = new TransactionMQProducer("tpg");
    3.  producer.setNamesrvAddr("rocketmqOS:9876"); 5

        6 /\*\*

2.  \* 定义一个线程池
3.  \* @param corePoolSize 线程池中核心线程数量
4.  \* @param maximumPoolSize 线程池中最多线程数
5.  \* @param keepAliveTime 这是一个时间。当线程池中线程数量大于核心线程数量是，
...
-   @param threadFactory 线程工厂
    ...
    ExecutorService executorService = new ThreadPoolExecutor(2, 5, TimeUnit.SECONDS,

    new ArrayBlockingQueue\<Runnable\>(2000), new

![](media/5d42a04bca91941dfe4463ac890390c4.png)ThreadFactory() {

1.  @Override
2.  public Thread newThread(Runnable r) {
3.  Thread thread = new Thread(r);
4.  thread.setName("client-transaction-msg-check-thread");
5.  return thread;
...
6.  // 为生产者指定一个线程池
7.  producer.setExecutorService(executorService);
8.  // 为生产者添加事务监听器
9.  producer.setTransactionListener(new ICBCTransactionListener()); 30

    31 producer.start(); 32

    33 String[] tags = {"TAGA","TAGB","TAGC"};

    34 for (int i = 0; i \< 3; i++) {

10. byte[] body = ("Hi," + i).getBytes();
11. Message msg = new Message("TTopic", tags[i], body);
12. // 发送事务消息
13. // 第二个参数用于指定在执行本地事务时要使用的业务参数
14. SendResult sendResult =

    producer.sendMessageInTransaction(msg,null);

15. System.out.println("发送结果为：" + sendResult.getSendStatus());
...
1.  public static void main(String[] args) throws MQClientException {
2.  // 定义一个pull消费者
1.  ![](media/4cb5274a35ce7d25c309d865746db73e.png)// DefaultLitePullConsumer consumer = new
...
4.  // 指定nameServer
5.  consumer.setNamesrvAddr("rocketmqOS:9876");
6.  // 指定从第一条消息开始消费
...
7.  // 指定消费topic与tag
8.  consumer.subscribe("TTopic", "\*");
9.  // 指定采用“广播模式”进行消费，默认为“集群模式”
...
13. // 一旦broker中有了其订阅的消息就会触发该方法的执行，
14. // 其返回值为当前consumer消费的状态
15. @Override
...
19. System.out.println(msg); 28 }
20. // 返回消费状态：消费成功
21. return ConsumeConcurrentlyStatus.CONSUME_SUCCESS; 31 }
...
22. // 开启消费者消费
23. consumer.start();
24. System.out.println("Consumer Started"); 37 }
...
### ![](media/3c5620dfaadd64daa91a568f63e63488.png)发送限制

生产者进行消息发送时可以一次发送多条消息，这可以大大提升Producer的发送效率。不过需要注意以 下几点：

批量发送的消息必须具有相同的Topic

批量发送的消息必须具有相同的刷盘策略批量发送的消息不能是延时消息与事务消息

### 批量发送大小

默认情况下，一批发送的消息总大小不能超过4MB字节。如果想超出该值，有两种解决方案：

方案一：将批量消息进行拆分，拆分为若干不大于4M的消息集合分多次批量发送方案二：在Producer端与Broker端修改属性

\*\* Producer端需要在发送之前设置Producer的maxMessageSize属性

\*\* Broker端需要修改其加载的配置文件中的maxMessageSize属性**生产者发送的消息大小**

生产者通过send()方法发送的Message，并不是直接将Message序列化后发送到网络上的，而是通过这 个Message生成了一个字符串发送出去的。这个字符串由四部分构成：Topic、消息Body、消息日志

（占20字节），及用于描述消息的一堆属性key-value。这些属性中包含例如生产者地址、生产时间、 要发送的QueueId等。最终写入到Broker中消息单元中的数据都是来自于这些属性。

## 批量消费消息

### 修改批量属性

![](media/86499458be310092475a7686c7af26dc.png)

Consumer的MessageListenerConcurrently监听接口的consumeMessage()方法的第一个参数为消息列 表，但默认情况下每次只能消费一条消息。若要使其一次可以消费多条消息，则可以通过修改Consumer的consumeMessageBatchMaxSize属性来指定。不过，该值不能超过32。因为默认情况下消 费者每次可以拉取的消息最多是32条。若要修改一次拉取的最大值，则可通过修改Consumer的pullBatchSize属性来指定。
...
1.  // 消息列表分割器：其只会处理每条消息的大小不超4M的情况。
    1.  // 若存在某条消息，其本身大小大于4M，这个分割器无法处理，
    2.  // 其直接将这条消息构成一个子列表返回。并没有再进行分割
    3.  public class MessageListSplitter implements Iterator\<List\<Message\>\> {
    4.  // 指定极限值为4M
    5.  private final int SIZE_LIMIT = 4 \*1024 \* 1024;
    6.  // 存放所有要发送的消息
    7.  private final List\<Message\> messages;
    8.  // 要进行批量发送消息的小集合起始索引
    9.  private int currIndex;
    10. public MessageListSplitter(List\<Message\> messages) {
    11. this.messages = messages;

![](media/aa67f6a479eac7e060354c1049fa1a19.png)13 }

14

1.  @Override
2.  public boolean hasNext() {
3.  // 判断当前开始遍历的消息索引要小于消息总数
4.  return currIndex \< messages.size(); 19 }

    20

5.  @Override
6.  public List\<Message\> next() {
7.  int nextIndex = currIndex;
8.  // 记录当前要发送的这一小批次消息列表的大小
9.  int totalSize = 0;
10. for (; nextIndex \< messages.size(); nextIndex++) {
11. // 获取当前遍历的消息
12. Message message = messages.get(nextIndex);

    29

13. // 统计当前遍历的message的大小
14. int tmpSize = message.getTopic().length() + message.getBody().length;
15. Map\<String, String\> properties = message.getProperties();
16. for (Map.Entry\<String, String\> entry : properties.entrySet()) {
17. tmpSize += entry.getKey().length() + entry.getValue().length();

    35 }

    36 tmpSize = tmpSize + 20; 37

18. // 判断当前消息本身是否大于4M
19. if (tmpSize \> SIZE_LIMIT) {
20. if (nextIndex - currIndex == 0) {
21. nextIndex++;
...
22. if (tmpSize + totalSize \> SIZE_LIMIT) {
23. break;
24. } else {
25. totalSize += tmpSize; 50 }

    51

    52 } // end-for 53

26. // 获取当前messages列表的子集合[currIndex, nextIndex)
27. List\<Message\> subList = messages.subList(currIndex, nextIndex);
28. // 下次遍历的开始索引
29. currIndex = nextIndex;
...
![](media/e42ac64a2783908880c4decabdb7cfb6.png)
...
1.  public class BatchProducer {
    ...
    2.  DefaultMQProducer producer = new DefaultMQProducer("pg");
    3.  producer.setNamesrvAddr("rocketmqOS:9876");
    4.  // 指定要发送的消息的最大大小，默认是4M
    5.  // 不过，仅修改该属性是不行的，还需要同时修改broker加载的配置文件中的
    6.  // maxMessageSize属性
    7.  // producer.setMaxMessageSize(8 \* 1024 \* 1024);
    8.  producer.start(); 10
2.  // 定义要发送的消息集合
3.  List\<Message\> messages = new ArrayList\<\>(); 13 for (int i = 0; i \< 100; i++) {
4.  byte[] body = ("Hi," + i).getBytes();
5.  Message msg = new Message("someTopic", "someTag", body);
6.  messages.add(msg); 17 }

    18

7.  // 定义消息列表分割器，将消息列表分割为多个不超出4M大小的小列表
8.  MessageListSplitter splitter = new MessageListSplitter(messages);
9.  while (splitter.hasNext()) {
10. try {
11. List\<Message\> listItem = splitter.next();
12. producer.send(listItem);
13. } catch (Exception e) {
14. e.printStackTrace(); 27 }

    28 }

    29 producer.shutdown(); 30 }

    31 }

### 定义批量消息消费者

1 public class BatchConsumer { 2

1.  public static void main(String[] args) throws MQClientException {
2.  DefaultMQPushConsumer consumer = new DefaultMQPushConsumer("cg");
3.  consumer.setNamesrvAddr("rocketmqOS:9876");

![](media/8eb8742a180a74f0f1b766221d2754a0.png)6

consumer.setConsumeFromWhere(ConsumeFromWhere.CONSUME_FROM_FIRST_OFFSET

);

7 consumer.subscribe("someTopicA", "\*"); 8

1.  // 指定每次可以消费10条消息，默认为1
2.  consumer.setConsumeMessageBatchMaxSize(10);
3.  // 指定每次可以从Broker拉取40条消息，默认为32
4.  consumer.setPullBatchSize(40); 13
...
7.  for (MessageExt msg : msgs) {
8.  System.out.println(msg); 21 }
9.  // 消费成功的返回结果
10. return ConsumeConcurrentlyStatus.CONSUME_SUCCESS;
11. // 消费异常时的返回结果
12. // return ConsumeConcurrentlyStatus.RECONSUME_LATER; 26 }
...
1.  consumer.start();
2.  System.out.println("Consumer Started"); 31 }
...
1.  ![](media/0997dcde879cf11021c4bec45f04d686.png)DefaultMQPushConsumer consumer = new DefaultMQPushConsumer("CID_EXAMPLE");
    1.  consumer.subscribe("TOPIC", "TAGA \|\| TAGB \|\| TAGC");
...
字符：必须用单引号包裹起来，比如：'abc' 布 尔 ：TRUE 或 FALSE NULL：特殊的常量，表示空

支持的运算符有：

数值比较：\>，\>=，\<，\<=，BETWEEN，= 字符比较：=，\<\>，IN
...
5.  producer.start();
6.  String[] tags = {"myTagA","myTagB","myTagC"}; 7 for (int i = 0; i \< 10; i++) {
...
byte[] body = ("Hi," + i).getBytes(); String tag = tags[i%tags.length];

Message msg = new Message("myTopic",tag,body); SendResult sendResult = producer.send(msg); System.out.println(sendResult);
...
![](media/5cf9b9f48334979c20663e3e8fac37b1.png)

### 定义Tag过滤Consumer

1 public class FilterByTagConsumer { 2

1.  public static void main(String[] args) throws Exception {
2.  DefaultMQPushConsumer consumer = new DefaultMQPushConsumer("pg");
3.  consumer.setNamesrvAddr("rocketmqOS:9876"); 6
...
1.  consumer.subscribe("myTopic", "myTagA \|\| myTagB");
...
6.  for (MessageExt me:msgs){
7.  System.out.println(me); 15 }

    16 return ConsumeConcurrentlyStatus.CONSUME_SUCCESS; 17 }

    18 });

8.  consumer.start();
9.  System.out.println("Consumer Started"); 21 }
...
Message msg = new Message("myTopic", "myTag", body); msg.putUserProperty("age", i + "");
...
![](media/aefc3dbb1ed9349bb8110b34e3180212.png)

### 定义SQL过滤Consumer

1 public class FilterBySQLConsumer { 2
...
3.  consumer.setNamesrvAddr("rocketmqOS:9876"); 6
    ...
    8 consumer.subscribe("myTopic", MessageSelector.bySql("age between 0 and 6"));
...
7.  for (MessageExt me:msgs){
8.  System.out.println(me); 15 }
...
![](media/e44a64cc2ac7ed198293d881bf69f867.png)
...
消息重复在一般情况下不会发生，当出现消息量大、网络抖动，消息重复就会成为大概率事件producer主动重发、consumer负载变化（发生Rebalance，不会导致消息重复，但可能出现重复 消费）也会导致重复消息
...
1.  // 创建一个producer，参数为Producer Group名称
    ...
    2.  // 指定nameServer地址
    3.  producer.setNamesrvAddr("rocketmqOS:9876");
    4.  // 设置同步发送失败时重试发送的次数，默认为2次
    5.  producer.setRetryTimesWhenSendFailed(3);
    6.  // 设置发送超时时限为5s，默认3s
    7.  producer.setSendMsgTimeout(5000);
...
1.  ![](media/6cd175542bfae96984ae53439be0171f.png)方案一：*Producer*中维护某*JUC*的*Map*集合，其*key*是发生失败的时间戳，*value*为*Broker*实例。*Producer*中还维护着一个*Set*集合，其中存放着所有未发生发送异常的*Broker*实例。选择目标*Broker*是从该*Set*集合中选择的。再定义一个定时任务，定期从*Map*集合中将长期未发生发送 异常的*Broker*清理出去，并添加到*Set*集合。
1.  方案二：为*Producer*中的*Broker*实例添加一个标识，例如是一个*AtomicBoolean*属性。只要该*Broker*上发生过发送异常，就将其置为*true*。选择目标*Broker*就是选择该属性值为*false*的*Broker*。再定义一个定时任务，定期将*Broker*的该属性置为*false*。
1.  方案三：为*Producer*中的*Broker*实例添加一个标识，例如是一个*AtomicLong*属性。只要该*Broker*上发生过发送异常，就使其值增一。选择目标*Broker*就是选择该属性值最小的*Broker*。若该值相同，采用轮询方式选择。
...
消息刷盘超时（Master或Slave）或slave不可用（slave在做数据同步时向master返回状态不是SEND_OK）时，默认是不会将消息尝试发送到其他Broker的。不过，对于重要消息可以通过在Broker 的配置文件设置retryAnotherBrokerWhenNotStoreOK属性为true来开启。

# 八、消息消费重试机制

## 顺序消息的消费重试

对于顺序消息，当Consumer消费消息失败后，为了保证消息的顺序性，其会自动不断地进行消息重 试，直到消费成功。消费重试默认间隔时间为1000毫秒。重试期间应用会出现消息消费被阻塞的情 况。
...
对于无序消息（普通消息、延时消息、事务消息），当Consumer消费消息失败时，可以通过设置返回 状态达到消息重试的效果。不过需要注意，无序消息的重试只对集群消费方式生效，广播消费方式不 提供失败重试特性。即对于广播消费，消费失败后，失败消息不再重试，继续消费后续消息。
...
![](media/0a6815019df3e1022d5ebde65746cfb1.png)
...
1.  DefaultMQPushConsumer consumer = new DefaultMQPushConsumer("cg");
    1.  // 修改消费重试次数
    2.  consumer.setMaxReconsumeTimes(10);
...
对于*Consumer Group*，若仅修改了一个*Consumer*的消费重试次数，则会应用到该*Group*中所有其它*Consumer*实例。若出现多个*Consumer*均做了修改的情况，则采用覆盖方式生效。即最后被 修改的值会覆盖前面设置的值。

## ![](media/19d042daa69656e9ed5c0bf2d138bc49.jpeg)重试队列
...
1.  只有当出现需要进行重试消费的消息时，才会为该消费者组创建重试队列
...
![](media/89c180331029d7351ed4129d36efa5a4.jpeg)

集群消费方式下，消息消费失败后若希望消费重试，则需要在消息监听器接口的实现中明确进行如下三 种方式之一的配置：

方式1：返回ConsumeConcurrentlyStatus.RECONSUME_LATER（推荐） 方式2：返回Null
...
## ![](media/77538bebaff1e8525989c6c38b0fef00.png)什么是死信队列

当一条消息初次消费失败，消息队列会自动进行消费重试；达到最大重试次数后，若消费依然失败，则 表明消费者在正常情况下无法正确地消费该消息，此时，消息队列不会立刻将消息丢弃，而是将其发送 到该消费者对应的特殊队列中。这个队列就是死信队列（Dead-Letter Queue，DLQ），而其中的消息则称为死信消息（Dead-Letter Message，DLM）。
